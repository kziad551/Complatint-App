  ...
  Future<void> handleSubmit() async {
    ...
    if (voiceFile != null) {
      voiceFileId = await uploadMediaToDirectus(File(voiceFile!.path));
    }

final complaintData = {
   'Complaint_Subcategory': int.tryParse(widget.subCategory), // Ensure subCategory is a valid integer
  ...
  'user': userId,
 'governorate_name': selectedGovernorate ?? '', // Governorate ID
'district': selectedGovernorate ?? '',
  'street_name_or_number': streetNameOrNumber?.isEmpty ?? true ? null : streetNameOrNumber,
  if (imageFileId != null) 'image': imageFileId,
  if (videoFileId != null) 'video': videoFileId,
  if (voiceFileId != null) 'voice': voiceFileId,
};

    const apiUrl = 'https://complaint.top-wp.com/items/Complaint';
    try {
      print("Payload: ${jsonEncode(complaintData)}"); // Debug payload
  ...
  children: [
    ...
    Expanded(
      child: TextField(
        ...
        onChanged: (value) {
          setState(() {
            selectedDistrict = value; // Update the district value
          });
        },
      ),
    ),
